...
function generateTreeListToggle(oldTreeListToggle, dataTree) {
  ...
  let mapTree = (nodes) => {
    nodes.forEach((node) => {
      ...
      if (!newTreeListToggle.has(node.get('id'))) {
        newTreeListToggle = newTreeListToggle.set(node.get('id'), false);
      }
    });
  }
  mapTree(dataTree);
...
function TreeListReducer(state = initialState, action) {
  switch(action.type) {
    ...
    case UPDATE_TREE_LIST_TOGGLE:
      return state
        .set('treeListToggle', generateTreeListToggle(state.get('treeListToggle'), action.tree));
    default:
      return state;
  }
}
